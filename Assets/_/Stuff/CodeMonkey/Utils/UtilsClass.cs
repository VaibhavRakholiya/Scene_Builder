...
namespace CodeMonkey.Utils {
    ...
    public static class UtilsClass {
        ...
        public static TextMesh CreateWorldText(Transform parent, string text, Vector3 localPosition, int fontSize, Color color, TextAnchor textAnchor, TextAlignment textAlignment, int sortingOrder) {
            //💰💰💰Creating Grid Visuals from already present gameobject.
            GameObject gameObject = new GameObject("World_Text", typeof(TextMesh));
            GameObject grid_visual_prefab = GameObject.FindGameObjectWithTag("Grid_Visuals");
            if(text != "Cannot Build Here!")
            {
                GameObject grid_visual = GameObject.Instantiate(grid_visual_prefab);
                grid_visual.transform.SetParent(parent, false);
                grid_visual.tag = "Untagged";
                grid_visual.transform.localPosition = localPosition;
            }
            Transform transform = gameObject.transform;
            transform.SetParent(parent, false);
            transform.localPosition = localPosition + new Vector3(1000f, 1000f, 1000f);
